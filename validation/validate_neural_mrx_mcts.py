...
MRX_STRATEGIES = ("gnn", "neural_mcts", "mcts", "random")
...
def _play_detective_phase(game, engine, detective_strategy, detective_policy):
    ...
    for detective_id in range(game.num_detectives):
        if detective_strategy == "heuristic":
            ...
            detective_policy.play_detective_turn(
                game,
                engine.belief_state,
                detective_id,
            )
...
def _choose_mrx_action(
    ...
    if mrx_strategy == "gnn":
        ...
        legal = ticket is None or destination in legal_before.get(ticket, [])
        game.x_automated_turn(destination, ticket)
        info = gnn_mrx_policy.last_action_info or {}
        return destination, ticket, legal, info

    if mrx_strategy == "neural_mcts":
        destination, ticket = neural_mcts_policy.choose_action(game, engine.belief_state)
        legal = ticket is None or destination in legal_before.get(ticket, [])
        game.x_automated_turn(destination, ticket)
        info = neural_mcts_policy.last_search_info or {}
        return destination, ticket, legal, info
...
def play_game(
    ...
    if neural_mcts_policy is not None:
        neural_mcts_policy.reset()
    ...
    while True:
        ...
        destination, ticket, legal, info = _choose_mrx_action(
            game,
            engine,
            mrx_strategy,
            mcts_policy,
        ...
        if mrx_strategy == "neural_mcts" and info:
            selected = info.get("selected") or {}
            selected_visits.append(int(selected.get("visits", 0)))
            root_actions.append(len(info.get("actions") or []))
        ...
        if mrx_strategy == "neural_mcts":
            neural_mcts_policy.observe_mrx_move(game, ticket)
    ...
    return {
        ...
        "avg_selected_visits": (
            float(np.mean(selected_visits)) if selected_visits else None
        ),
        "avg_root_actions": float(np.mean(root_actions)) if root_actions else None,
    }
...
def evaluate_suite(
        ...
        for i in range(n_games):
            ...
            results.append(
                play_game(
                    ...
                    neural_mcts_policy=neural_mcts_policy,
    ...
    selected_visits = [
        r["avg_selected_visits"]
        for r in results
        if r["avg_selected_visits"] is not None
    ]
    root_actions = [
        r["avg_root_actions"]
        for r in results
        if r["avg_root_actions"] is not None
    ]
    ...
    return {
        ...
        "neural_mcts_simulations": (
            None if neural_mcts_simulations is None else int(neural_mcts_simulations)
        ),
...
def parse_suite(raw):
    ...
    if len(parts) not in (4, 5, 6):
        raise argparse.ArgumentTypeError(
            "Suite must be name:detectives:mrx:games[:param1[:param2]]"
        )
    ...
    if mrx == "mcts":
        ...
        if len(parts) >= 5:
            explorations = int(parts[4])
        if len(parts) == 6:
            simulations = int(parts[5])
    elif mrx == "neural_mcts":
        simulations = 64
        if len(parts) >= 5:
            simulations = int(parts[4])
    return name, detectives, mrx, int(games), explorations, simulations


def main():
    parser = argparse.ArgumentParser(
        description="Validate Neural Mr.X MCTS against frozen detective policies."
    )
    parser.add_argument("--mrx-checkpoint", default=None, help="Path to Mr.X checkpoint")
    parser.add_argument(
        "--detective-checkpoint",
        default=None,
        help="Path to detective GNN checkpoint",
    )
    ...
    suites = args.suite or [
        ("mrx_ppo_v1_argmax", "gnn", "gnn", 20, None, None),
        ("neural_mcts_32", "gnn", "neural_mcts", 10, None, 32),
        ("neural_mcts_64", "gnn", "neural_mcts", 10, None, 64),
        ("mcts_15_25", "gnn", "mcts", 10, 15, 25),
        ("mcts_30_50", "gnn", "mcts", 10, 30, 50),
    ]

    needs_detective_gnn = any(s[1] == "gnn" for s in suites)
    needs_mrx_gnn = any(s[2] == "gnn" for s in suites)
    needs_neural_mcts = any(s[2] == "neural_mcts" for s in suites)

    detective_checkpoint = args.detective_checkpoint
    if needs_detective_gnn or needs_neural_mcts:
    ...
    if needs_mrx_gnn or needs_neural_mcts:
    ...
    detective_policy = (
        GNNDetectiveEngine(checkpoint_path=detective_checkpoint, device=args.device)
        if needs_detective_gnn
        else None
    )
    gnn_mrx_policy = (
        GNNMrXEngine(checkpoint_path=mrx_checkpoint, device=args.device)
        if needs_mrx_gnn
        else None
    )

    neural_policies = {}
    for _, _, mrx, _, _, simulations in suites:
        if mrx != "neural_mcts":
            continue
        key = int(simulations or 64)
        if key not in neural_policies:
            neural_policies[key] = NeuralMrXMCTSEngine(
    ...
    results = {
        ...
        "mrx_checkpoint_metadata": (
            gnn_mrx_policy.metadata
            if gnn_mrx_policy is not None
            else next(iter(neural_policies.values())).mrx_policy.metadata
            if neural_policies
            else {}
        ),
        ...
        "detective_checkpoint_metadata": (
            detective_policy.metadata
            if detective_policy is not None
            else next(iter(neural_policies.values())).detective_policy.metadata
            if neural_policies
            else {}
        ),
        "suites": {},
    ...
    for idx, suite in enumerate(suites):
        ...
        if mrx == "neural_mcts":
            print(f"  Neural MCTS simulations={simulations}")
        ...
        suite_result = evaluate_suite(
            ...
            gnn_mrx_policy=gnn_mrx_policy,
            neural_mcts_policy=(
                neural_policies.get(int(simulations or 64))
                if mrx == "neural_mcts"
                else None
            ),
            mrx_explorations=explorations,
            mrx_simulations=simulations if mrx == "mcts" else None,
            neural_mcts_simulations=simulations if mrx == "neural_mcts" else None,
